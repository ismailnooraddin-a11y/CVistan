// src/lib/translations.ts
export const translations = {
  en: {
    app: "CVistan",
    tagline: "Build your dream CV in minutes 🚀",
    selectLang: "Select Language",
    signIn: "Sign In",
    signUp: "Sign Up",
    back: "Back",
    next: "Next",
    step: "Step",
    of: "of",
    preview: "Preview",
    select: "Use This",
    personal: "Personal Info",
    summary: "About You",
    experience: "Experience",
    education: "Education",
    certifications: "Certifications",
    skills: "Skills",
    languages: "Languages",
    design: "Choose Design",
    review: "Review & Export",
    name: "Full Name",
    nameT: "Your name as it appears on your CV",
    nameP: "Ahmed Akram",
    title: "Job Title",
    titleT: "Position you're applying for",
    titleP: "Senior Accountant",
    email: "Email",
    emailT: "Professional email address",
    emailP: "ahmed.akram@email.com",
    phone: "Phone",
    phoneT: "With country code",
    phoneP: "+964 750 XXX XXXX",
    location: "Location",
    locationT: "City and country",
    locationP: "Erbil, Kurdistan, Iraq",
    dob: "Date of Birth",
    dobT: "Your birth date",
    dobP: "Select your date of birth",
    socialLinks: "Social & Portfolio Links",
    socialLinksT: "Add your professional profiles (optional)",
    addLink: "Add Link",
    linkedin: "LinkedIn",
    linkedinP: "linkedin.com/in/ahmedakram",
    github: "GitHub",
    githubP: "github.com/ahmedakram",
    portfolio: "Portfolio Website",
    portfolioP: "ahmedakram.com",
    twitter: "X (Twitter)",
    twitterP: "x.com/ahmedakram",
    instagram: "Instagram",
    instagramP: "instagram.com/ahmedakram",
    behance: "Behance",
    behanceP: "behance.net/ahmedakram",
    removeSocial: "Remove",
    opt: "optional",
    photo: "Add Photo",
    photoT: "Professional headshot recommended",
    sumTitle: "Professional Summary",
    sumT: "2-3 sentences about yourself",
    sumP: "Detail-oriented and highly analytical Senior Accountant with over 5 years of experience in financial reporting, budgeting, and tax preparation. Proven track record of improving financial processes, reducing costs by 15%, and ensuring compliance with international accounting standards (IFRS). Skilled in QuickBooks, SAP, and advanced Excel. Seeking to leverage my expertise in financial analysis and team leadership to contribute to a dynamic organization's growth and profitability.",
    sumH: "💡 Mention your experience, skills, and career goals",
    addExp: "Add Experience",
    noExp: "No experience yet",
    noExpH: "Add jobs, internships, or volunteer work",
    company: "Company",
    companyP: "ABC Financial Services",
    startD: "Start Date",
    endD: "End Date",
    current: "I currently work here",
    present: "Present",
    month: "Month",
    year: "Year",
    desc: "Responsibilities",
    descT: "What did you do?",
    descP: "• Prepared monthly, quarterly, and annual financial statements\n• Managed accounts payable/receivable for 50+ clients\n• Reduced operational costs by 15% through process optimization\n• Led a team of 3 junior accountants",
    addEdu: "Add Education",
    noEdu: "No education yet",
    noEduH: "Add degrees, diplomas, or courses",
    degree: "Degree",
    degreeP: "Bachelor's in Accounting",
    inst: "Institution",
    instP: "Salahaddin University",
    gradD: "Graduation Date",
    gpa: "GPA/Grade",
    gpaP: "3.8/4.0",
    gpaH: "Optional - add if impressive",
    thesis: "Thesis/Project",
    thesisP: "Financial Analysis of SMEs in Kurdistan Region",
    thesisH: "Optional - if relevant",
    addCert: "Add Certification",
    noCert: "No certifications yet",
    noCertH: "Add professional certifications, licenses, or training",
    certName: "Certification Name",
    certNameP: "PMP - Project Management Professional",
    certIssuer: "Issuing Organization",
    certIssuerP: "Project Management Institute (PMI)",
    certIssueD: "Issue Date",
    certExpiryD: "Expiry Date",
    certNoExpiry: "This certification does not expire",
    certCredId: "Credential ID",
    certCredIdP: "ABC123XYZ",
    certCredUrl: "Credential URL",
    certCredUrlP: "https://verify.pmi.org/...",
    certMode: "Training Mode",
    certOnline: "Online",
    certInPerson: "In-Person",
    skillsT: "What are you good at?",
    skillP: "Type skill and press Enter",
    skillH: "💡 Add both technical and soft skills",
    suggest: "Suggested:",
    suggestedSkills: [
      "Microsoft Office", "Communication", "Leadership", "Problem Solving", "Teamwork",
      "Project Management", "Time Management", "Critical Thinking", "Customer Service", "Sales",
      "Marketing", "Data Analysis", "Public Speaking", "Negotiation", "Adaptability",
      "Creativity", "Attention to Detail", "Organization", "Decision Making", "Conflict Resolution",
      "Networking", "Research", "Writing", "Presentation Skills", "Strategic Planning",
      "Budgeting", "Social Media", "Microsoft Excel", "Google Workspace", "CRM Software"
    ],
    langT: "Being multilingual is valuable!",
    addLang: "Add Language",
    langP: "English",
    native: "Native",
    fluent: "Fluent",
    conv: "Conversational",
    basic: "Basic",
    designT: "Pick a template that fits your style",
    done: "Your CV is Ready! 🎉",
    downloadT: "Download your CV in your preferred format",
    edit: "Edit",
    restart: "Start Over",
    reqField: "This field is required",
    reqName: "Please enter your full name",
    reqTitle: "Please enter your job title",
    reqPhone: "Please enter your phone number",
    reqSection: "Please add at least one: Experience, Education, or Skill",
    validationError: "Please fix the errors before continuing",
    exportTitle: "Export Options",
    downloadPdf: "Download PDF",
    downloadWord: "Download Word",
    generating: "Generating...",
    shareTitle: "Share via Messaging",
    shareDesc: "Send your CV directly via WhatsApp or Telegram",
    shareWhatsapp: "WhatsApp",
    shareTelegram: "Telegram",
    saveTitle: "Save Your CV?",
    saveDesc: "Create an account to save your CV and access it anytime",
    saveYes: "Yes, create account",
    saveNo: "No, just download",
    signupTitle: "Create Your Account",
    signupDesc: "Save your CV and access it from anywhere",
    signinTitle: "Welcome Back",
    signinDesc: "Sign in to access your saved CVs",
    signupName: "Full Name",
    signupEmail: "Email Address",
    signupPhone: "Phone Number",
    signupPassword: "Password",
    signupConfirm: "Confirm Password",
    signupBtn: "Create Account & Save CV",
    signinBtn: "Sign In",
    signupSuccess: "Account created! Your CV has been saved.",
    signinSuccess: "Welcome back!",
    passwordMismatch: "Passwords do not match",
    noAccount: "Don't have an account?",
    haveAccount: "Already have an account?",
    myCVs: "My CVs",
    createNew: "Create New CV",
    lastEdited: "Last edited",
    viewCV: "View",
    editCV: "Edit",
    deleteCV: "Delete",
    noCVs: "No CVs yet. Create your first one!",
    morgan: "Morgan",
    morganD: "Bold header with elegant sidebar",
    catrine: "Catrine",
    catrineD: "Photo-focused professional layout",
    sarah: "Sarah",
    sarahD: "Clean two-column minimal design",
    olivia: "Olivia",
    oliviaD: "Modern with color accents",
    months: ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  },
  ar: {
    app: "CVistan",
    tagline: "ابنِ سيرتك الذاتية في دقائق 🚀",
    selectLang: "اختر لغتك",
    signIn: "تسجيل الدخول",
    signUp: "إنشاء حساب",
    back: "رجوع",
    next: "التالي",
    step: "خطوة",
    of: "من",
    preview: "معاينة",
    select: "استخدم هذا",
    personal: "المعلومات الشخصية",
    summary: "نبذة عنك",
    experience: "الخبرة العملية",
    education: "التعليم",
    certifications: "الشهادات المهنية",
    skills: "المهارات",
    languages: "اللغات",
    design: "اختر التصميم",
    review: "المراجعة والتصدير",
    name: "الاسم الكامل",
    nameT: "اسمك كما سيظهر في سيرتك",
    nameP: "أحمد أكرم",
    title: "المسمى الوظيفي",
    titleT: "الوظيفة التي تتقدم لها",
    titleP: "محاسب أول",
    email: "البريد الإلكتروني",
    emailT: "بريد إلكتروني احترافي",
    emailP: "ahmed.akram@email.com",
    phone: "رقم الهاتف",
    phoneT: "مع رمز الدولة",
    phoneP: "+964 750 XXX XXXX",
    location: "الموقع",
    locationT: "المدينة والدولة",
    locationP: "أربيل، كردستان، العراق",
    dob: "تاريخ الميلاد",
    dobT: "تاريخ ميلادك",
    dobP: "اختر تاريخ ميلادك",
    socialLinks: "روابط التواصل الاجتماعي",
    socialLinksT: "أضف حساباتك المهنية (اختياري)",
    addLink: "أضف رابط",
    linkedin: "لينكد إن",
    linkedinP: "linkedin.com/in/ahmedakram",
    github: "جيت هب",
    githubP: "github.com/ahmedakram",
    portfolio: "الموقع الشخصي",
    portfolioP: "ahmedakram.com",
    twitter: "إكس (تويتر)",
    twitterP: "x.com/ahmedakram",
    instagram: "إنستغرام",
    instagramP: "instagram.com/ahmedakram",
    behance: "بيهانس",
    behanceP: "behance.net/ahmedakram",
    removeSocial: "إزالة",
    opt: "اختياري",
    photo: "أضف صورة",
    photoT: "يُفضل صورة احترافية",
    sumTitle: "الملخص المهني",
    sumT: "2-3 جمل عن نفسك",
    sumP: "محاسب أول دقيق وتحليلي مع أكثر من 5 سنوات من الخبرة في إعداد التقارير المالية والميزانيات وإعداد الضرائب. سجل حافل في تحسين العمليات المالية وخفض التكاليف بنسبة 15% وضمان الامتثال للمعايير المحاسبية الدولية (IFRS). ماهر في QuickBooks و SAP و Excel المتقدم. أسعى للاستفادة من خبرتي في التحليل المالي وقيادة الفريق للمساهمة في نمو وربحية المؤسسة.",
    sumH: "💡 اذكر خبرتك ومهاراتك وأهدافك المهنية",
    addExp: "أضف خبرة",
    noExp: "لا توجد خبرات بعد",
    noExpH: "أضف الوظائف أو التدريب أو العمل التطوعي",
    company: "الشركة",
    companyP: "شركة ABC للخدمات المالية",
    startD: "تاريخ البدء",
    endD: "تاريخ الانتهاء",
    current: "أعمل هنا حالياً",
    present: "حتى الآن",
    month: "الشهر",
    year: "السنة",
    desc: "المسؤوليات",
    descT: "ماذا كانت مهامك؟",
    descP: "• إعداد البيانات المالية الشهرية والربعية والسنوية\n• إدارة الحسابات الدائنة والمدينة لأكثر من 50 عميل\n• خفض التكاليف التشغيلية بنسبة 15%\n• قيادة فريق من 3 محاسبين",
    addEdu: "أضف تعليم",
    noEdu: "لا يوجد تعليم بعد",
    noEduH: "أضف الشهادات أو الدبلومات أو الدورات",
    degree: "الشهادة",
    degreeP: "بكالوريوس محاسبة",
    inst: "المؤسسة التعليمية",
    instP: "جامعة صلاح الدين",
    gradD: "تاريخ التخرج",
    gpa: "المعدل",
    gpaP: "4.5 من 5",
    gpaH: "اختياري - أضفه إذا كان مميزاً",
    thesis: "الرسالة/المشروع",
    thesisP: "التحليل المالي للشركات الصغيرة والمتوسطة في إقليم كردستان",
    thesisH: "اختياري - إذا كان ذا صلة",
    addCert: "أضف شهادة",
    noCert: "لا توجد شهادات بعد",
    noCertH: "أضف الشهادات المهنية أو التراخيص أو التدريب",
    certName: "اسم الشهادة",
    certNameP: "PMP - محترف إدارة المشاريع",
    certIssuer: "الجهة المانحة",
    certIssuerP: "معهد إدارة المشاريع (PMI)",
    certIssueD: "تاريخ الإصدار",
    certExpiryD: "تاريخ الانتهاء",
    certNoExpiry: "هذه الشهادة لا تنتهي صلاحيتها",
    certCredId: "رقم الاعتماد",
    certCredIdP: "ABC123XYZ",
    certCredUrl: "رابط التحقق",
    certCredUrlP: "https://verify.pmi.org/...",
    certMode: "نوع التدريب",
    certOnline: "عن بُعد",
    certInPerson: "حضوري",
    skillsT: "ما الذي تجيده؟",
    skillP: "اكتب المهارة واضغط Enter",
    skillH: "💡 أضف المهارات التقنية والشخصية",
    suggest: "مقترحات:",
    suggestedSkills: [
      "مايكروسوفت أوفيس", "التواصل", "القيادة", "حل المشكلات", "العمل الجماعي",
      "إدارة المشاريع", "إدارة الوقت", "التفكير النقدي", "خدمة العملاء", "المبيعات",
      "التسويق", "تحليل البيانات", "الخطابة", "التفاوض", "التكيف",
      "الإبداع", "الدقة", "التنظيم", "اتخاذ القرارات", "حل النزاعات",
      "بناء العلاقات", "البحث", "الكتابة", "العروض التقديمية", "التخطيط الاستراتيجي",
      "إعداد الميزانيات", "وسائل التواصل الاجتماعي", "إكسل", "جوجل", "برامج CRM"
    ],
    langT: "إتقان عدة لغات ميزة قيمة!",
    addLang: "أضف لغة",
    langP: "العربية",
    native: "لغة أم",
    fluent: "طلاقة",
    conv: "محادثة",
    basic: "أساسي",
    designT: "اختر قالباً يناسب أسلوبك",
    done: "سيرتك الذاتية جاهزة! 🎉",
    downloadT: "حمّل سيرتك بالصيغة المفضلة",
    edit: "تعديل",
    restart: "ابدأ من جديد",
    reqField: "هذا الحقل مطلوب",
    reqName: "الرجاء إدخال الاسم الكامل",
    reqTitle: "الرجاء إدخال المسمى الوظيفي",
    reqPhone: "الرجاء إدخال رقم الهاتف",
    reqSection: "الرجاء إضافة واحد على الأقل: خبرة أو تعليم أو مهارة",
    validationError: "الرجاء تصحيح الأخطاء قبل المتابعة",
    exportTitle: "خيارات التصدير",
    downloadPdf: "تحميل PDF",
    downloadWord: "تحميل Word",
    generating: "جاري الإنشاء...",
    shareTitle: "مشاركة عبر الرسائل",
    shareDesc: "أرسل سيرتك مباشرة عبر واتساب أو تيليجرام",
    shareWhatsapp: "واتساب",
    shareTelegram: "تيليجرام",
    saveTitle: "حفظ السيرة الذاتية؟",
    saveDesc: "أنشئ حساباً لحفظ سيرتك والوصول إليها في أي وقت",
    saveYes: "نعم، أنشئ حساب",
    saveNo: "لا، تحميل فقط",
    signupTitle: "إنشاء حسابك",
    signupDesc: "احفظ سيرتك وادخل إليها من أي مكان",
    signinTitle: "مرحباً بعودتك",
    signinDesc: "سجل دخولك للوصول إلى سيرك الذاتية",
    signupName: "الاسم الكامل",
    signupEmail: "البريد الإلكتروني",
    signupPhone: "رقم الهاتف",
    signupPassword: "كلمة المرور",
    signupConfirm: "تأكيد كلمة المرور",
    signupBtn: "إنشاء الحساب وحفظ السيرة",
    signinBtn: "تسجيل الدخول",
    signupSuccess: "تم إنشاء الحساب! تم حفظ سيرتك الذاتية.",
    signinSuccess: "مرحباً بعودتك!",
    passwordMismatch: "كلمات المرور غير متطابقة",
    noAccount: "ليس لديك حساب؟",
    haveAccount: "لديك حساب بالفعل؟",
    myCVs: "سيرتي الذاتية",
    createNew: "إنشاء سيرة جديدة",
    lastEdited: "آخر تعديل",
    viewCV: "عرض",
    editCV: "تعديل",
    deleteCV: "حذف",
    noCVs: "لا توجد سير ذاتية. أنشئ أولى سيرك!",
    morgan: "مورغان",
    morganD: "رأس جريء مع شريط جانبي أنيق",
    catrine: "كاترين",
    catrineD: "تصميم احترافي يركز على الصورة",
    sarah: "سارة",
    sarahD: "تصميم بسيط بعمودين",
    olivia: "أوليفيا",
    oliviaD: "عصري مع لمسات ملونة",
    months: ["يناير", "فبراير", "مارس", "أبريل", "مايو", "يونيو", "يوليو", "أغسطس", "سبتمبر", "أكتوبر", "نوفمبر", "ديسمبر"],
  },
  ku: {
    app: "CVistan",
    tagline: "CVەکەت لە چەند خولەکدا دروست بکە 🚀",
    selectLang: "زمان هەڵبژێرە",
    signIn: "چوونەژوورەوە",
    signUp: "تۆمارکردن",
    back: "گەڕانەوە",
    next: "دواتر",
    step: "هەنگاو",
    of: "لە",
    preview: "پێشبینین",
    select: "ئەمە بەکاربهێنە",
    personal: "زانیاری کەسی",
    summary: "دەربارەی تۆ",
    experience: "ئەزموون",
    education: "خوێندن",
    certifications: "بڕوانامە پیشەییەکان",
    skills: "لێهاتووییەکان",
    languages: "زمانەکان",
    design: "دیزاین هەڵبژێرە",
    review: "پێداچوونەوە و هەناردە",
    name: "ناوی تەواو",
    nameT: "ناوەکەت وەک چۆن لە CVدا دەردەکەوێت",
    nameP: "ئەحمەد ئەکرەم",
    title: "ناونیشانی کار",
    titleT: "ئەو پۆستەی بۆی پێشکەش دەکەیت",
    titleP: "ژمێریاری باڵا",
    email: "ئیمەیڵ",
    emailT: "ئیمەیڵی پڕۆفیشناڵ",
    emailP: "ahmed.akram@email.com",
    phone: "ژمارەی تەلەفۆن",
    phoneT: "لەگەڵ کۆدی وڵات",
    phoneP: "+964 750 XXX XXXX",
    location: "شوێن",
    locationT: "شار و وڵات",
    locationP: "هەولێر، کوردستان، عێراق",
    dob: "بەرواری لەدایکبوون",
    dobT: "بەرواری لەدایکبوونت",
    dobP: "بەرواری لەدایکبوونت هەڵبژێرە",
    socialLinks: "لینکەکانی کۆمەڵایەتی",
    socialLinksT: "هەژمارە پڕۆفیشناڵەکانت زیاد بکە (هەڵبژاردەیی)",
    addLink: "لینک زیاد بکە",
    linkedin: "لینکدئین",
    linkedinP: "linkedin.com/in/ahmedakram",
    github: "گیتهەب",
    githubP: "github.com/ahmedakram",
    portfolio: "وێبسایتی کەسی",
    portfolioP: "ahmedakram.com",
    twitter: "ئێکس (تویتەر)",
    twitterP: "x.com/ahmedakram",
    instagram: "ئینستاگرام",
    instagramP: "instagram.com/ahmedakram",
    behance: "بیهانس",
    behanceP: "behance.net/ahmedakram",
    removeSocial: "سڕینەوە",
    opt: "هەڵبژاردەیی",
    photo: "وێنە زیاد بکە",
    photoT: "وێنەی پڕۆفیشناڵ پێشنیار دەکرێت",
    sumTitle: "پوختەی پڕۆفیشناڵ",
    sumT: "2-3 ڕستە دەربارەی خۆت",
    sumP: "ژمێریارێکی باڵا و وردبینم لەگەڵ زیاتر لە 5 ساڵ ئەزموون لە ڕاپۆرتی دارایی، بودجە، و ئامادەکردنی باج.",
    sumH: "💡 ئەزموون و لێهاتوویی و ئامانجەکانت باس بکە",
    addExp: "ئەزموون زیاد بکە",
    noExp: "هێشتا ئەزموون نییە",
    noExpH: "کار، ڕاهێنان، یان کاری خۆبەخش زیاد بکە",
    company: "کۆمپانیا",
    companyP: "کۆمپانیای ABC بۆ خزمەتگوزاری دارایی",
    startD: "بەرواری دەستپێکردن",
    endD: "بەرواری کۆتایی",
    current: "ئێستا لێرە کار دەکەم",
    present: "تا ئێستا",
    month: "مانگ",
    year: "ساڵ",
    desc: "بەرپرسیاریەتییەکان",
    descT: "چیت کرد؟",
    descP: "• ئامادەکردنی ڕاپۆرتی دارایی\n• بەڕێوەبردنی حسابەکان\n• کەمکردنەوەی تێچوو بە 15%",
    addEdu: "خوێندن زیاد بکە",
    noEdu: "هێشتا خوێندن نییە",
    noEduH: "بڕوانامە، دیپلۆم، یان کۆرس زیاد بکە",
    degree: "بڕوانامە",
    degreeP: "بەکالۆریۆس لە ژمێریاری",
    inst: "دامەزراوەی خوێندن",
    instP: "زانکۆی سەلاحەددین",
    gradD: "بەرواری دەرچوون",
    gpa: "نمرە/GPA",
    gpaP: "85%",
    gpaH: "هەڵبژاردەیی - زیادی بکە ئەگەر باش بوو",
    thesis: "تێزی/پڕۆژە",
    thesisP: "شیکاری دارایی کۆمپانیا بچووک و مامناوەندەکان",
    thesisH: "هەڵبژاردەیی - ئەگەر پەیوەندیدار بوو",
    addCert: "بڕوانامە زیاد بکە",
    noCert: "هێشتا بڕوانامە نییە",
    noCertH: "بڕوانامە پیشەیی، مۆڵەت، یان ڕاهێنان زیاد بکە",
    certName: "ناوی بڕوانامە",
    certNameP: "PMP - پسپۆڕی بەڕێوەبردنی پڕۆژە",
    certIssuer: "دامەزراوەی دەرکەر",
    certIssuerP: "پەیمانگەی بەڕێوەبردنی پڕۆژە (PMI)",
    certIssueD: "بەرواری دەرکردن",
    certExpiryD: "بەرواری بەسەرچوون",
    certNoExpiry: "ئەم بڕوانامەیە بەسەرناچێت",
    certCredId: "ژمارەی پشتڕاستکردنەوە",
    certCredIdP: "ABC123XYZ",
    certCredUrl: "لینکی پشتڕاستکردنەوە",
    certCredUrlP: "https://verify.pmi.org/...",
    certMode: "شێوازی ڕاهێنان",
    certOnline: "ئۆنلاین",
    certInPerson: "ئامادەبوون",
    skillsT: "لە چیدا شارەزایت؟",
    skillP: "لێهاتوویی بنووسە و Enter دابگرە",
    skillH: "💡 لێهاتوویی تەکنیکی و نەرم زیاد بکە",
    suggest: "پێشنیارکراو:",
    suggestedSkills: [
      "مایکرۆسۆفت ئۆفیس", "پەیوەندی", "سەرکردایەتی", "چارەسەرکردنی کێشە", "کاری تیمی",
      "بەڕێوەبردنی پڕۆژە", "بەڕێوەبردنی کات", "بیرکردنەوەی ڕەخنەیی", "خزمەتگوزاری کڕیار", "فرۆشتن"
    ],
    langT: "زانینی چەند زمانێک بەهادارە!",
    addLang: "زمان زیاد بکە",
    langP: "کوردی",
    native: "زمانی دایک",
    fluent: "شارەزا",
    conv: "قسەکردن",
    basic: "بنەڕەتی",
    designT: "تەمپلەیتێک هەڵبژێرە کە لەگەڵ ستایلەکەت بگونجێت",
    done: "CVەکەت ئامادەیە! 🎉",
    downloadT: "CVەکەت بە فۆرماتی دڵخوازت دابگرە",
    edit: "دەستکاری",
    restart: "دووبارە دەست پێبکەوە",
    reqField: "ئەم خانەیە پێویستە",
    reqName: "تکایە ناوی تەواو بنووسە",
    reqTitle: "تکایە ناونیشانی کار بنووسە",
    reqPhone: "تکایە ژمارەی تەلەفۆن بنووسە",
    reqSection: "تکایە لانی کەم یەکێک زیاد بکە: ئەزموون، خوێندن، یان لێهاتوویی",
    validationError: "تکایە هەڵەکان چارەسەر بکە پێش بەردەوامبوون",
    exportTitle: "هەڵبژاردنەکانی هەناردە",
    downloadPdf: "داگرتنی PDF",
    downloadWord: "داگرتنی Word",
    generating: "دروستکردن...",
    shareTitle: "هاوبەشکردن لە ڕێگەی نامە",
    shareDesc: "CVەکەت ڕاستەوخۆ بنێرە لە ڕێگەی واتساپ یان تێلیگرام",
    shareWhatsapp: "واتساپ",
    shareTelegram: "تێلیگرام",
    saveTitle: "CVەکەت پاشەکەوت بکەیت؟",
    saveDesc: "هەژمارێک دروست بکە بۆ پاشەکەوتکردن و دەستگەیشتن لە هەر کاتێکدا",
    saveYes: "بەڵێ، هەژمار دروست بکە",
    saveNo: "نەخێر، تەنها داگرتن",
    signupTitle: "هەژمارەکەت دروست بکە",
    signupDesc: "CVەکەت پاشەکەوت بکە و لە هەر شوێنێکەوە دەستی پێبگە",
    signinTitle: "بەخێربێیتەوە",
    signinDesc: "بچۆرەوە بۆ دەستگەیشتن بە CVەکانت",
    signupName: "ناوی تەواو",
    signupEmail: "ئیمەیڵ",
    signupPhone: "ژمارەی تەلەفۆن",
    signupPassword: "وشەی نهێنی",
    signupConfirm: "دووبارەکردنەوەی وشەی نهێنی",
    signupBtn: "دروستکردنی هەژمار و پاشەکەوتکردنی CV",
    signinBtn: "چوونەژوورەوە",
    signupSuccess: "هەژمار دروست کرا! CVەکەت پاشەکەوت کرا.",
    signinSuccess: "بەخێربێیتەوە!",
    passwordMismatch: "وشە نهێنییەکان یەک ناگرنەوە",
    noAccount: "هەژمارت نییە؟",
    haveAccount: "هەژمارت هەیە؟",
    myCVs: "CVەکانم",
    createNew: "CV نوێ دروست بکە",
    lastEdited: "دوایین دەستکاری",
    viewCV: "بینین",
    editCV: "دەستکاری",
    deleteCV: "سڕینەوە",
    noCVs: "هێشتا CV نییە. یەکەمیان دروست بکە!",
    morgan: "مۆرگان",
    morganD: "سەرپەڕەی بەهێز لەگەڵ لاپەڕەی جوان",
    catrine: "کاترین",
    catrineD: "دیزاینی پڕۆفیشناڵ کە تیشک دەخاتە سەر وێنە",
    sarah: "سارا",
    sarahD: "دیزاینی سادە بە دوو ستوون",
    olivia: "ئۆلیڤیا",
    oliviaD: "مۆدێرن لەگەڵ ڕەنگی جوان",
    months: ["کانوونی دووەم", "شوبات", "ئازار", "نیسان", "ئایار", "حوزەیران", "تەممووز", "ئاب", "ئەیلوول", "تشرینی یەکەم", "تشرینی دووەم", "کانوونی یەکەم"],
  }
};

export type TranslationKey = keyof typeof translations.en;
